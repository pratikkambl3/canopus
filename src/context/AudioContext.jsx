...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'LOAD_TRACKS': {
      ...
      if (!tracks.length) return { ...state, tracks };
      const queue = tracks.map(t => t.id);
      const currentTrackId = state.currentTrackId || queue[0];
      const queueIndex = Math.max(0, queue.indexOf(currentTrackId));
      return { ...state, tracks, queue, queueIndex, currentTrackId };
    }
...
export function AudioProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const onError    = () => dispatch({ type: 'SET_ERROR', payload: 'Unable to play this record.' });
  ...
  /* ── Sync track change → load new src ── */
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio) return;
    if (state.currentTrackId === prevTrackIdRef.current) return;
  ...
  }, [state.currentTrackId, state.tracks]); // eslint-disable-line react-hooks/exhaustive-deps

  /* ── Sync isPlaying → play / pause ── */
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio || prevIsPlayingRef.current === state.isPlaying) return;
